...
//    }
    ...
    int maxTextWidth = (int) (width - paceH * 2);
    String[] lines = wrapText(text, fm1, maxTextWidth, 2);
    int lineHeight = fm1.getHeight();
    int textY = (int) (recY + fm1.getAscent() + spaceV);
    for (int i = 0; i < lines.length; i++) {
        g2.drawString(lines[i], (float) recX, textY + i * lineHeight);
    }
...
private String[] wrapText(String text, FontMetrics fm, int maxWidth, int maxLines) {
    ...
    for (String word : words) {
        ...
        if (fm.stringWidth(currentLine.toString()) > maxWidth) {
            if (lines.size() + 1 < maxLines) {
                ...
                // Add ellipsis (...) to the last line if it exceeds the maximum width
                String lastLine = currentLine.toString();
                int ellipsisWidth = fm.stringWidth("...");
                while (fm.stringWidth(lastLine + "...") > maxWidth - ellipsisWidth) {
